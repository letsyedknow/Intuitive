...
FROM
	(SELECT BIRD_ID,
			...
			HABITAT_ID,
			WINGSPAN,
			DENSE_RANK() OVER(PARTITION BY HABITAT_ID
							  ORDER BY WINGSPAN DESC) AS BIRD_RANK
		FROM BIRD) B
WHERE B.BIRD_RANK <= 3 ;
...
SELECT AR.GAME_ID,
	AR.USER_ID,
	SUM(AR.LAST_POINTS) AS TOTAL_POINTS,
	DENSE_RANK() OVER ( ORDER BY SUM(AR.LAST_POINTS)) AS CREDIT_RANK --CONSIDERING LESS POINT HIGHER RANK
FROM
	(--SUM OF POINTS AFTER EACH ROUND
SELECT G.GAME_ID,
			G.ROUND,
			G.USER_ID,
			SUM(B.POINT) AS LAST_POINTS
		FROM
			(SELECT GAME_ID,
					ROUND,
					USER_ID,
					UNNEST(CARD_IN_HAND) AS CARD_ASSET_ID
				FROM GAME
				WHERE GAME_ID IN
						(--CONSIDERING THE LATEST COMPLETED GAME
 							SELECT MAX(GAME_ID)
							FROM GAME
							WHERE IS_COMPLETED = TRUE ) ) G
		JOIN BIRD B ON G.CARD_ASSET_ID = B.CARD_ASSET_ID
		GROUP BY 1,2,
			3) AR
GROUP BY 1,2;
...
SELECT AF.USER_ID,
	...
	U.PREVIOUS_POINT + AF.TOTAL_POINTS AS TOTAL_POINTS,--AUTO SUM OF ALL PLAYER
 	DENSE_RANK() OVER (ORDER BY (U.PREVIOUS_POINT + AF.TOTAL_POINTS) DESC) AS CREDIT,
	CURRENT_TIMESTAMP
FROM
...
SELECT AR.GAME_ID,
			AR.USER_ID,
			SUM(AR.LAST_POINTS) AS TOTAL_POINTS,
			DENSE_RANK() OVER (ORDER BY SUM(AR.LAST_POINTS) DESC) AS CREDIT_RANK
		FROM
			(--SUM OF POINTS AFTER EACH ROUND
			 SELECT G.GAME_ID,
					G.ROUND,
				...
				FROM
					(SELECT GAME_ID,
							ROUND,
							USER_ID,
							UNNEST(CARD_IN_HAND) AS CARD_ASSET_ID --STORED AS AN ARRAY TO REDUCE REDUNDENCY
						FROM GAME
						WHERE GAME_ID IN
								(--CONSIDERING THE LATEST COMPLETED GAME
 SELECT MAX(GAME_ID)
...
JOIN
	(SELECT USER_ID,
			USER_NAME,
			MAX(TOTAL_POINTS) AS PREVIOUS_POINT
		FROM USERS
		GROUP BY 1,2)U ON AF.USER_ID = U.USER_ID;
